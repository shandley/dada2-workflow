...
determine_parameter_bounds <- function(quality_profiles, target_amplicon_size = NULL, 
  ...
  min_trunc_f <- max(100, floor(max_forward_cycle * 0.4)) # Minimum 100bp or 40% of read
  min_trunc_r <- max(100, floor(max_reverse_cycle * 0.4)) # Minimum 100bp or 40% of read
...
evaluate_parameters <- function(truncF, truncR, maxEE_F, maxEE_R) {
  ...
  # Calculate stringency score (we prefer more stringent filtering when possible)
  # Lower maxEE values are more stringent, but we want a higher score for more stringency
  maxEE_total <- maxEE_F + maxEE_R
  stringency_score <- 15 - maxEE_total  # Higher score for lower total maxEE
  
  # Calculate read length score (we prefer to keep more of the read when quality allows)
  ...
  length_score <- (length_proportion_f + length_proportion_r) * 15  # Scale to contribute meaningfully
...
dada2_optimization_function <- function(truncF, truncR, maxEE_F, maxEE_R) {
  ...
  local_evaluate_parameters <- function(truncF, truncR, maxEE_F, maxEE_R) {
    ...
    tryCatch({
      ...
      # Simplified scoring for parallel optimization
      # Focus mostly on retention rate with a small penalty for excessive maxEE
      retention_score <- retention_rate * 100  # Scale up to make it significant
      maxEE_penalty <- (maxEE_F + maxEE_R) * 2  # Small penalty for high error rates
      ...
      # Simple final score
      final_score <- retention_score - maxEE_penalty + amplicon_score
...
if (has_pbo) {
  ...
  # Extract best parameters - the object structure from bayesOpt is different from pbayesopt
  best_params <- optim_results$scoreSummary[which.max(optim_results$scoreSummary$Score),]
  truncLen_forward <- as.integer(best_params$truncF)
  truncLen_reverse <- as.integer(best_params$truncR)
  maxEE_forward <- best_params$maxEE_F
  maxEE_reverse <- best_params$maxEE_R
...
validate_optimized_parameters <- function(truncF, truncR, maxEE_F, maxEE_R, 
  ...
  # Make sure truncation lengths are valid
  cat(sprintf("Validating with truncation lengths: forward=%d, reverse=%d\n", truncF, truncR))
  
  # Filter with optimized parameters - make sure to pass truncLen as a vector of length 2
...
cat("Debug: truncLen_forward =", truncLen_forward, "truncLen_reverse =", truncLen_reverse, "\n")
cat("Debug: maxEE_forward =", maxEE_forward, "maxEE_reverse =", maxEE_reverse, "\n")
...
truncLen_vec <- c(as.integer(truncLen_forward), as.integer(truncLen_reverse))
maxEE_vec <- c(maxEE_forward, maxEE_reverse)

cat("Using truncLen =", paste(truncLen_vec, collapse=", "), "\n")
cat("Using maxEE =", paste(maxEE_vec, collapse=", "), "\n")
...
# Check if parameters are valid before creating the summary
cat("Creating parameter summary with debug info:\n")
print(list(
  maxEE_forward = maxEE_forward,
  maxEE_reverse = maxEE_reverse,
  class_forward = class(maxEE_forward),
  class_reverse = class(maxEE_reverse)
))

# Convert parameters to numeric if needed
if (!exists("maxEE_forward") || is.null(maxEE_forward)) {
  cat("maxEE_forward not found, setting default value\n")
  maxEE_forward <- 2.0
} else if (!is.numeric(maxEE_forward)) {
  cat("Converting maxEE_forward to numeric\n")
  maxEE_forward <- as.numeric(maxEE_forward)
  if (is.na(maxEE_forward)) maxEE_forward <- 2.0  # Default value if conversion fails
}
if (!exists("maxEE_reverse") || is.null(maxEE_reverse)) {
  cat("maxEE_reverse not found, setting default value\n")
  maxEE_reverse <- 2.0
} else if (!is.numeric(maxEE_reverse)) {
  cat("Converting maxEE_reverse to numeric\n")
  maxEE_reverse <- as.numeric(maxEE_reverse)
  if (is.na(maxEE_reverse)) maxEE_reverse <- 2.0  # Default value if conversion fails
}

# Create parameter summary table in a safer way with explicit values
# First, collect all parameter values with error handling

# Platform info
if (exists("quality_profiles") && is.list(quality_profiles) && "platform" %in% names(quality_profiles)) {
  platform_value <- quality_profiles$platform
} else {
  platform_value <- "Unknown"
}

# Forward read length
if (exists("quality_profiles") && is.list(quality_profiles) && "max_forward_cycle" %in% names(quality_profiles)) {
  forward_length <- quality_profiles$max_forward_cycle
} else {
  forward_length <- NA
}

# Reverse read length
if (exists("quality_profiles") && is.list(quality_profiles) && "max_reverse_cycle" %in% names(quality_profiles)) {
  reverse_length <- quality_profiles$max_reverse_cycle
} else {
  reverse_length <- NA
}

# Create the parameter table in pieces to avoid length mismatches
# First, create the platform and read length info table with error handling
tryCatch({
  platform_df <- data.frame(
    Parameter = c("Platform", "Forward Read Length", "Reverse Read Length"),
    Value = c(platform_value, forward_length, reverse_length),
  ...
  cat("Error creating platform table:", conditionMessage(e), "\n")
  platform_df <<- data.frame(
    Parameter = c("Platform"),
    Value = c("Unknown"),
    Notes = c("Sequencing platform information not available"),
    stringsAsFactors = FALSE
  )
})

# Next, create the truncation info table with error handling
tryCatch({
  # Ensure truncation values are available
  if (!exists("truncLen_forward")) truncLen_forward <- NA
  if (!exists("truncLen_reverse")) truncLen_reverse <- NA
  
  trunc_df <- data.frame(
    Parameter = c("Forward Truncation Length", "Reverse Truncation Length"),
    Value = c(truncLen_forward, truncLen_reverse),
    Notes = c("Bayesian optimized truncation position", "Bayesian optimized truncation position"),
    stringsAsFactors = FALSE
  )
}, error = function(e) {
  cat("Error creating truncation table:", conditionMessage(e), "\n")
  trunc_df <<- data.frame(Parameter = character(0), Value = character(0), Notes = character(0))
})

# Next, create the maxEE table with much stronger error handling
tryCatch({
  # Safe conversion to character strings, to avoid any sprintf errors
  if (exists("maxEE_forward")) {
    forward_maxee_str <- tryCatch({
      if (is.numeric(maxEE_forward)) {
        sprintf("%.1f", maxEE_forward)
      } else {
        as.character(maxEE_forward)
      }
    }, error = function(e) "2.0")  # Default if conversion fails
  } else {
    forward_maxee_str <- "2.0"  # Default value
  }
  
  if (exists("maxEE_reverse")) {
    reverse_maxee_str <- tryCatch({
      if (is.numeric(maxEE_reverse)) {
        sprintf("%.1f", maxEE_reverse)
      } else {
        as.character(maxEE_reverse)
      }
    }, error = function(e) "2.0")  # Default if conversion fails
  } else {
    reverse_maxee_str <- "2.0"  # Default value
  }
  
  # Create the data frame
  maxee_df <- data.frame(
    Parameter = c("Forward maxEE", "Reverse maxEE"),
    Value = c(forward_maxee_str, reverse_maxee_str),
    Notes = c("Bayesian optimized maximum expected errors", "Bayesian optimized maximum expected errors"),
    stringsAsFactors = FALSE  # Prevent automatic conversion to factors
  )
}, error = function(e) {
  # If anything fails, create an empty data frame
  cat("Error creating maxEE table:", conditionMessage(e), "\n")
  maxee_df <<- data.frame(Parameter = character(0), Value = character(0), Notes = character(0))
})

# Finally, create the overlap table with error handling
tryCatch({
  # Check if both values exist and create a complete table only if they do
  if (exists("expected_amplicon_size") && exists("expected_overlap")) {
    overlap_df <- data.frame(
      Parameter = c("Expected Amplicon Size", "Expected Overlap"),
      Value = c(expected_amplicon_size, expected_overlap),
      Notes = c("Expected amplicon size based on primer", "Expected overlap after truncation"),
      stringsAsFactors = FALSE
    )
  } else if (exists("expected_amplicon_size")) {
    # Only amplicon size exists
    overlap_df <- data.frame(
      Parameter = c("Expected Amplicon Size"),
      Value = c(expected_amplicon_size),
      Notes = c("Expected amplicon size based on primer"),
      stringsAsFactors = FALSE
    )
  } else if (exists("expected_overlap")) {
    # Only overlap exists
    overlap_df <- data.frame(
      Parameter = c("Expected Overlap"),
      Value = c(expected_overlap),
      Notes = c("Expected overlap after truncation"),
      stringsAsFactors = FALSE
    )
  } else {
    # Neither exists
  ...
  cat("Error creating overlap table:", conditionMessage(e), "\n")
  overlap_df <<- data.frame(Parameter = character(0), Value = character(0), Notes = character(0))
})

# Combine all tables with rbind in a safe way
tryCatch({
  # Make sure all data frames have the same structure before rbind
  all_dfs <- list(platform_df, trunc_df, maxee_df, overlap_df)
  valid_dfs <- list()
  
  for (df in all_dfs) {
    if (is.data.frame(df) && ncol(df) >= 3 && nrow(df) > 0) {
      # Ensure column names match
      colnames(df) <- c("Parameter", "Value", "Notes")
      valid_dfs <- c(valid_dfs, list(df))
    }
  }
  
  # If we have valid data frames, combine them
  if (length(valid_dfs) > 0) {
    param_summary <- do.call(rbind, valid_dfs)
    kable(param_summary, caption = "Bayesian Optimized DADA2 Parameters")
  } else {
    # Create a basic fallback table
    fallback_df <- data.frame(
      Parameter = c("Status"),
      Value = c("Parameter optimization completed"),
      Notes = c("Details not available due to an error in parameter summary generation")
    )
    kable(fallback_df, caption = "DADA2 Parameter Optimization Status")
  }
}, error = function(e) {
  # If all else fails, show a simple message
  cat("Error generating parameter summary table:", conditionMessage(e), "\n")
  cat("Parameter optimization process completed, but summary table could not be generated.\n")
})

# Create a separate validation results table
if (exists("validation") && is.list(validation)) {
  # Check if validation has the required components
  has_retention_rate <- "retention_rate" %in% names(validation) && !is.null(validation$retention_rate)
  has_sample_retention <- "sample_retention" %in% names(validation) && !is.null(validation$sample_retention) && length(validation$sample_retention) > 0
  
  if (has_retention_rate && has_sample_retention) {
    ...
    cat("Validation results incomplete. Some metrics may not be available.\n")
    
    # Create a simplified validation table with only available metrics
    params <- c()
    values <- c()
    notes <- c()
    
    if (has_retention_rate) {
      params <- c(params, "Overall Read Retention")
      values <- c(values, paste0(validation$retention_rate, "%"))
      notes <- c(notes, "Percentage of reads retained after filtering")
    }
    ...
    if (length(params) > 0) {
      ...
      kable(validation_summary, caption = "Partial Validation Results")
    }
  }
}
...
# Final parameters for the rest of the workflow
cat("\nFinal Bayesian optimized parameters for filtering:\n")
...
bayesian_optimization_results <- list(
  ...
  platform = quality_profiles$platform,
  parameters = list(
