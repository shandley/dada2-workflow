...
if (Sys.info()["sysname"] == "Darwin") {  # Check if on macOS
  ...
  is_apple_silicon <- grepl("arm64", arch_check)
  
  if (is_apple_silicon) {
    cat("Detected Apple Silicon (M-series) processor. Applying optimized settings.\n")
  }
}

# Use progressive parallelization strategy with Apple Silicon optimizations
if (is_apple_silicon) {
  # For Apple Silicon, leverage efficiency cores for background tasks
  # and performance cores for compute-intensive tasks
  # M-series often has 4-10 cores with mix of performance and efficiency
  perf_cores <- min(num_cores * 0.6, 8)  # Estimate performance cores as 60% of total, max 8
  
  # Use slightly more workers on Apple Silicon as they handle parallel loads efficiently
  workers_to_use <- min(ceiling(perf_cores), num_cores - 1)
  cat(sprintf("Apple Silicon optimization: Using %d workers (estimated from %d total cores)\n", 
              workers_to_use, num_cores))
  
  # Apple Silicon benefits from multisession with custom scheduling
  plan(multisession, workers = workers_to_use)
  
  # Increase memory limits for Apple Silicon (unified memory architecture)
  # M-series chips share RAM efficiently between CPU and GPU
  mem_limit <- 6 * 1024^3  # 6GB for M-series (adjust based on total RAM)
  options(future.globals.maxSize = mem_limit)
  cat(sprintf("Memory allocation optimized for Apple Silicon: %.1fGB\n", mem_limit/1024^3))
  
  # Additional Apple Silicon optimizations
  # These environment variables can help BLAS/LAPACK operations on M-series
  Sys.setenv(VECLIB_MAXIMUM_THREADS = as.character(workers_to_use))
  
  # Consider enabling Apple's Accelerate framework if available
  # This helps with matrix operations that DADA2 uses extensively
  ...
  options(future.globals.maxSize = 4 * 1024^3)  # 4GB limit for globals
} else if (num_cores >= 4) {
  # For mid-range systems, use multisession with fewer workers
  workers_to_use <- min(num_cores - 1, 4)
  cat(sprintf("Detected %d cores. Using multisession with %d workers.\n", num_cores, workers_to_use))
  plan(multisession, workers = workers_to_use)
  options(future.globals.maxSize = 3 * 1024^3)  # 3GB limit for globals
} else {
...
# Apple Silicon optimization: adjust sample count for error learning
if (exists("is_apple_silicon") && is_apple_silicon) {
  # M-series chips can handle more samples efficiently due to unified memory and faster cores
  MAX_ERROR_LEARN_SAMPLES <- 60  # Increase sample count for Apple Silicon
  cat(sprintf("Apple Silicon optimization: Increasing error learning sample count to %d\n", 
...
if(length(filtFs) > MAX_ERROR_LEARN_SAMPLES) {
  ...
  # Apple Silicon optimized parameters for learnErrors
  nbases <- if (exists("is_apple_silicon") && is_apple_silicon) 1e9 else 1e8
  
  # Learn forward error rates using subset with M-series optimizations
  cat("Learning error rates for forward reads...\n")
  errF <- learnErrors(filtFs[error_learn_samples], multithread = TRUE, 
                      randomize = TRUE, verbose = TRUE, nbases = nbases)
  
  # Learn reverse error rates using subset
  cat("Learning error rates for reverse reads...\n")
  errR <- learnErrors(filtRs[error_learn_samples], multithread = TRUE, 
                      randomize = TRUE, verbose = TRUE, nbases = nbases)
  
  # Ensure garbage collection runs after each step - optimized for Apple Silicon
  if (exists("is_apple_silicon") && is_apple_silicon) {
    # Less aggressive GC for Apple Silicon
    invisible(gc(FALSE))
  } else {
    # Standard GC for other platforms
    gc()
  }
} else {
  ...
  # Force garbage collection between error learning steps - optimized for Apple Silicon
  if (exists("is_apple_silicon") && is_apple_silicon) {
    # Less aggressive GC for Apple Silicon
    invisible(gc(FALSE))
  } else {
...
if (length(derepFs) > 20 || is_multi_run) {
  ...
  # Apple Silicon specific batch size tuning for sample inference
  if (exists("is_apple_silicon") && is_apple_silicon) {
    # M-series chips handle larger batch sizes efficiently due to unified memory
    # and Neural Engine that can accelerate some matrix operations
    batch_size <- min(15, ceiling(length(derepFs)/3))
    cat(sprintf("Apple Silicon optimization: Using larger sample inference batch size of %d samples\n", 
                batch_size))
  }
  ...
  mem_info <- tryCatch({
    if (.Platform$OS.type == "unix") {
      ...
      # Adjust memory allocation per sample based on chip architecture
      mem_per_sample_gb <- if (exists("is_apple_silicon") && is_apple_silicon) 0.4 else 0.5
      
      # Adjust batch size based on available memory (rough heuristic)
      mem_based_batch <- max(1, floor(free_mem_gb / mem_per_sample_gb))
    ...
    # If memory detection fails, use architecture-specific default
    if (exists("is_apple_silicon") && is_apple_silicon) {
      cat("Memory detection failed, using Apple Silicon default batch size\n")
    } else {
  ...
  if(is_multi_run && exists("file_to_run")) {
    ...
    for(run in run_names) {
      ...
      # Force garbage collection with Apple Silicon optimizations
      if (exists("is_apple_silicon") && is_apple_silicon) {
        # Less aggressive GC on Apple Silicon (more efficient memory management)
        # Only run full GC at the end of each run
        if (run == run_names[length(run_names)]) {
          gc(full = TRUE, verbose = FALSE)
        } else {
          # Use a lighter GC for intermediate runs
          invisible(gc(FALSE, verbose = FALSE))
        }
      } else {
        # Standard GC for other architectures
        gc()
...
if (length(derepRs) > 20 || is_multi_run) {
  ...
  if (exists("is_apple_silicon") && is_apple_silicon) {
    # M-series chips handle larger batch sizes efficiently due to unified memory
    batch_size <- min(15, ceiling(length(derepRs)/3))
    cat(sprintf("Apple Silicon optimization: Using larger sample inference batch size of %d samples\n", 
                batch_size))
  }
...
build_phylogenetic_tree <- function(seqs, method = "ML", bootstrap = 100, cores = NULL, 
  ...
  # Set number of cores for parallel processing with Apple Silicon optimization
  if (is.null(cores)) {
  ...
  # Apply Apple Silicon optimizations for ML model fitting
  if (exists("is_apple_silicon") && is_apple_silicon) {
...
if (can_build_trees) {
  ...
  # Adjust thresholds based on hardware (Apple Silicon can handle larger datasets)
  ...
  if (length(dna) > large_asv_threshold) {
    ...
    # Apple Silicon can efficiently handle more bootstrap replicates
    if (exists("is_apple_silicon") && is_apple_silicon) {
      bootstrap_reps <- 150  # More bootstrap replicates for better tree support
