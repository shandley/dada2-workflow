...
# Check if output directory parameter exists, otherwise use default
...
# Check if output file parameter exists, otherwise use default
output_file_base <- if (exists("params") && 
                        !is.null(params$output_file) && 
                        params$output_file != "") {
  params$output_file
} else {
  "dada2_workflow_report"  # Default report name
}

# Create full output filename
output_file <- file.path(output_dir, paste0(output_file_base, "_", report_date))

# Generate the report
# First, get the current file path explicitly
current_file <- knitr::current_input()
if (is.null(current_file) || current_file == "") {
  # If not available, use a hardcoded path
  current_file <- "dada2_workflow_optimize.Rmd"
  cat("Warning: Could not detect current file. Using", current_file, "instead.\n")
}

# Check output format and use a sensible default if needed
output_format <- if (exists("params") && 
                     !is.null(params$output_format) && 
                     params$output_format != "") {
  params$output_format
} else {
  "html_document"  # Use HTML as the default format
}
cat("Using output format:", output_format, "\n")

# Try to generate the report with better error trapping
...
# Check if the result is an error message (character string) or a path (success)
if (is.character(report_result) && 
    (startsWith(report_result, "Error:") || startsWith(report_result, "Warning:"))) {
  cat("Report generation issue:", report_result, "\n")
} else {
