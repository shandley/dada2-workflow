...
download_ref_db <- function(db_name) {
  ...
  # Setup for IDTAXA/DECIPHER SILVA classifier
    ...
    if (!file.exists(silva_idtaxa)) {
      ...
      temp_dir <- tempdir()
      download_url <- "https://www2.decipher.codes/data/Downloads/TrainingSets/SILVA_SSU_r138_2_2024.RData"
...
if (taxonomy_methods$IDTAXA_SILVA) {
  ...
  tryCatch({
    ...
    # Verify that the trainingSet object was loaded correctly
    if (!exists("trainingSet") || !inherits(trainingSet, "TrainingSet")) {
      cat("WARNING: trainingSet object not found or incorrect type after loading database.\n")
      
      # Check what was actually loaded
      loaded_objects <- ls()
      cat("Objects loaded from the database file:", paste(loaded_objects, collapse=", "), "\n")
      
      # Look for alternative training set objects
      possible_training_sets <- loaded_objects[grep("train|Training", loaded_objects, ignore.case=TRUE)]
      if (length(possible_training_sets) > 0) {
        cat("Possible training set objects:", paste(possible_training_sets, collapse=", "), "\n")
        # Try to use the first one that looks like a training set
        for (obj_name in possible_training_sets) {
          if (inherits(get(obj_name), "TrainingSet")) {
            cat("Using", obj_name, "as the training set.\n")
            trainingSet <- get(obj_name)
            break
          }
        }
      }
      
      # If still no training set, stop with error
      if (!exists("trainingSet") || !inherits(trainingSet, "TrainingSet")) {
        stop("Cannot proceed: Valid IDTAXA training set not found in the loaded database.")
      }
    } else {
      cat("IDTAXA training set loaded successfully!\n")
      # Display some information about the training set
      cat("Training set has", length(trainingSet$taxa), "reference sequences\n")
    }
  }, error = function(e) {
    cat("ERROR loading training set:", conditionMessage(e), "\n")
    cat("This is likely due to an issue with the training set file. Try downloading it again.\n")
    stop("Cannot proceed with IDTAXA classification due to training set loading error.")
  })
