...
# Install and load required packages for Bayesian Optimization
if (!requireNamespace("rBayesianOptimization", quietly = TRUE)) {
  cat("Installing rBayesianOptimization package for Bayesian Optimization...\n")
  install.packages("rBayesianOptimization")
}

# Check for ParBayesianOptimization - better for parallel processing but optional
has_pbo <- requireNamespace("ParBayesianOptimization", quietly = TRUE)
if (!has_pbo) {
  cat("For faster parallel optimization, consider installing: ParBayesianOptimization\n")
}
...
dada2_optimization_function <- function(truncF, truncR, maxEE_F, maxEE_R) {
  score <- evaluate_parameters(truncF, truncR, maxEE_F, maxEE_R)
...
# Determine the best optimization approach based on available packages
if (has_pbo && requireNamespace("ParBayesianOptimization", quietly = TRUE)) {
  # Use parallel Bayesian optimization if available
  ...
  n_cores <- ifelse(exists("hw_caps"), 
                   min(hw_caps$optimal_workers, parallel::detectCores() - 1),
                   min(4, parallel::detectCores() - 1))
  
  # Run parallel Bayesian optimization
  optim_results <- ParBayesianOptimization::pbayesopt(
  ...
  max_iter <- ifelse(exists("hw_caps") && hw_caps$cores >= 8, 20, 15)
...
validation <- validate_optimized_parameters(
  truncLen_forward, truncLen_reverse,
  maxEE_forward, maxEE_reverse,
  validate_size = 5
)

# Create a more comprehensive parameter summary table
param_summary <- data.frame(
  Parameter = c("Platform", "Forward Read Length", "Reverse Read Length", 
                ...
                "Overall Read Retention", "Min Sample Retention", "Max Sample Retention"),
  Value = c(quality_profiles$platform, 
            quality_profiles$max_forward_cycle,
            quality_profiles$max_reverse_cycle,
            truncLen_forward, 
            truncLen_reverse, 
            ...
            expected_overlap,
            paste0(validation$retention_rate, "%"),
