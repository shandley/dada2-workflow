...
summarize_taxonomy <- function(taxa_list) {
  ...
  for (method_name in names(taxa_list)) {
    ...
    # First ensure standardized taxonomy before calculating percentages
    std_taxa_df <- standardize_taxonomy(taxa_df, method_name)
    ...
    for (level in c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")) {
      percent <- round(sum(!is.na(std_taxa_df[[level]])) / nrow(std_taxa_df) * 100, 1)
