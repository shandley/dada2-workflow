...
The following chunk configures the workflow for either single-run or multi-run processing. The workflow can handle:

1. **Single-run mode**: All FASTQ files in the `data/` directory are processed together
2. **Multi-run mode**: Multiple sequencing runs in separate subdirectories are processed with run-specific error models
   - Each run is processed independently through the denoising step
   - Results are combined for downstream analysis
   - Run information is preserved for batch effect analysis

The configuration automatically:
- Detects if multi-run mode is enabled via parameters
- Locates the appropriate data directories
- Finds FASTQ files with flexible naming pattern detection
- Configures memory management for big datasets
- Sets up appropriate data structures for tracking run information

**To enable multi-run mode or big data mode**, modify the YAML header at the top of this document:

```yaml
params:
  multi_run: TRUE             # Set to TRUE for multi-run processing
  run_dir: "path/to/runs"     # Optional: specify directory containing run folders
  big_data: TRUE              # Set to TRUE for memory optimization with large datasets
```

- For **multi-run mode**, set `multi_run: TRUE` and optionally specify `run_dir` 
- For **big data mode**, set `big_data: TRUE` to enable memory-efficient processing
- You can also set these parameters when rendering the Rmd via command line:
  ```r
  rmarkdown::render("dada2_workflow_optimize.Rmd", params = list(multi_run = TRUE, big_data = TRUE))
  ```
...
download_ref_db <- function(db_name) {
  ...
  # Setup for IDTAXA/DECIPHER SILVA classifier
    ...
    # Use a consistent filename for the SILVA IDTAXA training set
    silva_idtaxa <- file.path("ref_db", "SILVA_SSU_r138_IDTAXA.RData")
    
    # Force download a fresh copy if we previously had issues
    force_download <- file.exists(silva_idtaxa) && 
                     (file.exists("results/idtaxa_unclassified_sequences.rds") || 
                      exists("trainingSet") && !inherits(trainingSet, "TrainingSet"))
    
    if (force_download) {
      cat("Previous IDTAXA issues detected. Re-downloading fresh SILVA training set...\n")
      if (file.exists(silva_idtaxa)) {
        # Rename the old file for backup
        backup_file <- paste0(silva_idtaxa, ".backup.", format(Sys.time(), "%Y%m%d%H%M%S"))
        file.rename(silva_idtaxa, backup_file)
        cat("Backed up previous training set to:", backup_file, "\n")
      }
    }
    
    if (!file.exists(silva_idtaxa) || force_download) {
      cat("Downloading SILVA SSU r138 for IDTAXA...\n")
      
      # Try multiple download sources in case one fails
      download_urls <- c(
        "https://github.com/benjjneb/dada2/raw/master/data/silva_nr99_v138.1_train_set.fa.gz",
        "https://zenodo.org/records/4587955/files/silva_nr99_v138.1_train_set.fa.gz",
        "https://www2.decipher.codes/Classification/TrainingSets/SILVA_SSU_r138_2019.RData"
      )
      ...
      for (download_url in download_urls) {
        ...
        cat("Trying download from:", download_url, "\n")
        tryCatch({
          # Set extended timeout
          ...
          # Download to temp file first to avoid corrupted downloads
          download.file(download_url, temp_file, mode = "wb")
          
          # Check if the download worked by trying to load it
          temp_env <- new.env()
          load(temp_file, envir = temp_env)
          
          # Verify trainingSet exists in the downloaded file
          if (!exists("trainingSet", envir = temp_env) || 
              !inherits(temp_env$trainingSet, "TrainingSet")) {
            cat("Downloaded file doesn't contain valid training set. Trying next source...\n")
            next
          }
          
          # If it loaded correctly, copy to final location
          file.copy(temp_file, silva_idtaxa, overwrite = TRUE)
          unlink(temp_file)
          
          cat("Download successful and verified!\n")
          download_success <- TRUE
          
        }, error = function(e) {
          cat("Error downloading from this source:", conditionMessage(e), "\n")
          cat("Trying next source...\n")
        })
      }
      
      if (!download_success) {
        cat("All download attempts failed.\n")
        cat("Please download the SILVA training set manually from one of these URLs:\n")
        for (url in download_urls) {
          cat("  -", url, "\n")
        }
        cat("and save it to:", silva_idtaxa, "\n")
        
        # As a fallback, try to create a training set from scratch if DECIPHER is available
        if (requireNamespace("DECIPHER", quietly = TRUE)) {
          cat("Attempting to create a basic IDTAXA training set from scratch...\n")
          
          # This is a last resort option - will be very slow but might work
          tryCatch({
            # Try to create a basic training set
            # First check if we have the SILVA_SEED_r138.RData file
            seed_file <- file.path("ref_db", "SILVA_SEED_r138.RData")
            seed_url <- "https://github.com/ANPD-Data/RefData/raw/main/SILVA_SEED_r138.RData"
            
            if (!file.exists(seed_file)) {
              cat("Downloading SILVA SEED alignment file...\n")
              download.file(seed_url, seed_file, mode = "wb")
            }
            
            # Load the seed alignment
            load(seed_file)
            
            if (!exists("SILVA_SEED_r138")) {
              stop("SILVA_SEED_r138 object not found in the loaded file")
            }
            
            # Create a simplified training set
            cat("Creating training set from SILVA SEED (this will take some time)...\n")
            trainingSet <- DECIPHER::LearnTaxa(SILVA_SEED_r138$train, 
                                             SILVA_SEED_r138$taxonomy)
            
            # Save the created training set
            save(trainingSet, file = silva_idtaxa)
            cat("Basic training set created successfully. This is not as comprehensive as the official version.\n")
            download_success <- TRUE
            
          }, error = function(e) {
            cat("Failed to create training set:", conditionMessage(e), "\n")
          })
        }
        
        if (!download_success) {
          stop("Unable to obtain a valid IDTAXA training set.")
        }
      }
    } else {
      cat("Using existing SILVA IDTAXA training set\n")
      
      # Verify the existing training set
      temp_env <- new.env()
      load_result <- tryCatch({
        load(silva_idtaxa, envir = temp_env)
        TRUE
      }, error = function(e) {
        cat("Error loading existing training set:", conditionMessage(e), "\n")
        FALSE
      })
      
      if (load_result && (!exists("trainingSet", envir = temp_env) || 
                          !inherits(temp_env$trainingSet, "TrainingSet"))) {
        cat("Existing file doesn't contain a valid training set. Will download fresh copy.\n")
        # Rename the old file for backup
        backup_file <- paste0(silva_idtaxa, ".corrupt.", format(Sys.time(), "%Y%m%d%H%M%S"))
        file.rename(silva_idtaxa, backup_file)
        cat("Backed up corrupt training set to:", backup_file, "\n")
        # Call the function recursively to download a new copy
        return(download_ref_db("IDTAXA_SILVA"))
      }
    }
...
```{r idtaxa-taxonomy}
# IDTAXA with SILVA database - Optimized and Debugged Implementation
if (taxonomy_methods$IDTAXA_SILVA) {
  cat("\n=== Method 2: IDTAXA with SILVA database (with Enhanced Debugging) ===\n")
  
  # Track if we used IDTAXA or had to fall back to DADA2's assignTaxonomy
  used_fallback <- FALSE
  ...
  tryCatch({
    ...
    cat("ERROR with IDTAXA database download:", conditionMessage(e), "\n")
    cat("Will fall back to standard DADA2 taxonomy assignment instead of IDTAXA\n")
    used_fallback <- TRUE
  })
  
  # Load the training set with proper error handling
  cat("Loading IDTAXA training set...\n")
  
  # Create a new environment to load the training set into
  # This prevents variable name collisions with existing objects
  training_env <- new.env()
  
  load_result <- tryCatch({
    # Load the training set into the isolated environment
    load(idtaxa_files$train, envir = training_env)
    TRUE
  }, error = function(e) {
    cat("ERROR loading training set file:", conditionMessage(e), "\n")
    cat("The file may be corrupted. Will attempt to download a fresh copy.\n")
    
    # Delete the corrupted file
    if (file.exists(idtaxa_files$train)) {
      backup_file <- paste0(idtaxa_files$train, ".corrupted.", format(Sys.time(), "%Y%m%d%H%M%S"))
      file.rename(idtaxa_files$train, backup_file)
      cat("Renamed corrupted file to:", backup_file, "\n")
    }
    
    # Try to download the file again
    new_files <- download_ref_db("IDTAXA_SILVA")
    
    # Try loading again with the new file
    tryCatch({
      load(new_files$train, envir = training_env)
      cat("Successfully loaded newly downloaded training set!\n")
      TRUE
    }, error = function(e2) {
      cat("ERROR: Failed to load even after re-downloading:", conditionMessage(e2), "\n")
      FALSE
    })
  })
  
  if (!load_result) {
    stop("Cannot proceed with IDTAXA classification due to training set loading errors.")
  }
  
  # List objects in the training set file
  training_objs <- ls(envir = training_env)
  cat("Objects in training set file:", paste(training_objs, collapse=", "), "\n")
  
  # Check if trainingSet exists in the loaded environment
  if ("trainingSet" %in% training_objs) {
    # Check that it's the right type
    if (inherits(training_env$trainingSet, "TrainingSet")) {
      # Copy to our main environment
      trainingSet <- training_env$trainingSet
      cat("IDTAXA training set loaded successfully!\n")
      
      # Display information about the training set
      if (!is.null(trainingSet$taxa) && length(trainingSet$taxa) > 0) {
        cat("Training set has", length(trainingSet$taxa), "reference sequences\n")
      } else {
        cat("WARNING: Training set loaded but appears to be empty or incomplete\n")
      }
    } else {
      cat("WARNING: Object named 'trainingSet' exists but is not a proper TrainingSet object\n")
      stop("Cannot proceed: Invalid training set object")
    }
  } else {
    # Look for alternative objects that might be the training set
    possible_training_sets <- grep("train|taxa|classifier", training_objs, ignore.case=TRUE, value=TRUE)
    
    if (length(possible_training_sets) > 0) {
      cat("Looking for alternative training set objects:", paste(possible_training_sets, collapse=", "), "\n")
      
      # Try each possible object
      for (obj_name in possible_training_sets) {
        obj <- get(obj_name, envir=training_env)
        if (inherits(obj, "TrainingSet")) {
          cat("Found valid training set in object:", obj_name, "\n")
          trainingSet <- obj
          break
        }
      }
    ...
    # If we still don't have a training set, try to build one if possible
    if (!exists("trainingSet") || !inherits(trainingSet, "TrainingSet")) {
      cat("No valid training set found in file. Attempting to rebuild...\n")
      
      # Check if DECIPHER is available
      if (!requireNamespace("DECIPHER", quietly = TRUE)) {
        stop("DECIPHER package is required but not available")
      }
      
      # Use alternative approach: convert DADA2 SILVA training set for use with IDTAXA
      cat("Attempting to use DADA2's SILVA training set as fallback...\n")
      
      # First check if we can use DADA2's built-in SILVA training set
      silva_train_dada2 <- system.file("extdata", "silva_nr99_v138.1_train_set.fa.gz", package="dada2")
      
      if (file.exists(silva_train_dada2) && nzchar(silva_train_dada2)) {
        cat("Found DADA2's SILVA training set:", silva_train_dada2, "\n")
        cat("Converting DADA2's SILVA training set to IDTAXA format...\n")
        
        tryCatch({
          # Convert DADA2's SILVA training set to IDTAXA format
          # First need to read in the DADA2 training set
          dada2_seqs <- Biostrings::readDNAStringSet(silva_train_dada2)
          
          # Extract taxonomy from the headers
          headers <- names(dada2_seqs)
          taxonomy <- sapply(headers, function(x) {
            # Format is typically: >AB01234.1.1 Bacteria;Proteobacteria;...
            tax_string <- strsplit(x, " ", fixed=TRUE)[[1]][2]
            return(tax_string)
          })
          
          # Fix names for DECIPHER
          names(dada2_seqs) <- gsub(" .*", "", names(dada2_seqs))
          
          # Make taxonomy suitable for DECIPHER
          tax_table <- data.frame(
            Row.names = names(dada2_seqs),
            Taxonomy = taxonomy
          )
          
          # Train IDTAXA model
          cat("Training IDTAXA with DADA2's SILVA data (this may take a while)...\n")
          trainingSet <- DECIPHER::LearnTaxa(dada2_seqs, tax_table$Taxonomy)
          
          # Save for future use
          save(trainingSet, file=idtaxa_files$train)
          cat("Successfully created and saved new IDTAXA training set!\n")
        }, error = function(e) {
          cat("Failed to convert DADA2's SILVA training set:", conditionMessage(e), "\n")
          
          # Last resort: skip IDTAXA and use DADA2's assignTaxonomy directly
          cat("WARNING: IDTAXA classification unavailable. Will fall back to DADA2's assignTaxonomy.\n")
          
          # Notify that we'll be skipping IDTAXA
          taxonomy_methods$IDTAXA_SILVA <- FALSE
          stop("Cannot create IDTAXA training set - will try DADA2's assignTaxonomy instead")
        })
      } else {
        # Try manually downloading a SILVA fasta file
        cat("Attempting to download SILVA reference directly...\n")
        silva_url <- "https://zenodo.org/records/4587955/files/silva_nr99_v138.1_train_set.fa.gz"
        temp_file <- tempfile(fileext = ".fa.gz")
        
        tryCatch({
          download.file(silva_url, temp_file, mode="wb")
          
          # Read sequences
          cat("Downloaded SILVA reference. Converting to IDTAXA format...\n")
          dada2_seqs <- Biostrings::readDNAStringSet(temp_file)
          
          # Extract taxonomy from headers
          headers <- names(dada2_seqs)
          taxonomy <- sapply(headers, function(x) {
            # Format is typically: >AB01234.1.1 Bacteria;Proteobacteria;...
          ...
          # Filter out NAs
          valid_idx <- !is.na(taxonomy)
          dada2_seqs <- dada2_seqs[valid_idx]
          ...
          # Train IDTAXA model
          cat("Training IDTAXA with downloaded SILVA data (this may take a while)...\n")
          trainingSet <- DECIPHER::LearnTaxa(dada2_seqs, taxonomy)
          
          # Save for future use
          save(trainingSet, file=idtaxa_files$train)
          cat("Successfully created and saved new IDTAXA training set!\n")
        }, error = function(e) {
          cat("Failed to create training set from downloaded data:", conditionMessage(e), "\n")
  ...
  # Final check to ensure we have a valid training set
  if (!exists("trainingSet") || !inherits(trainingSet, "TrainingSet")) {
    cat("ERROR: No valid IDTAXA training set available despite attempts to fix the issue\n")
    cat("Falling back to DADA2's assignTaxonomy method instead\n")
    used_fallback <- TRUE
  }
  
  # Use DADA2's assignTaxonomy as a fallback if IDTAXA failed
  if (used_fallback) {
    # Skip IDTAXA and use DADA2's assignTaxonomy directly
    cat("\n=== Using DADA2's assignTaxonomy as fallback ===\n")
    
    # First try to load SILVA reference from DADA2 package
    silva_train <- system.file("extdata", "silva_nr99_v138.1_train_set.fa.gz", package="dada2")
    silva_species <- system.file("extdata", "silva_species_assignment_v138.1.fa.gz", package="dada2")
    
    # Check if file exists within package
    if (!file.exists(silva_train) || !file.exists(silva_species)) {
      # Try downloading from online source if not in package
      cat("SILVA references not found in DADA2 package. Attempting to download...\n")
      ...
      if (!file.exists(silva_train)) {
        ...
        tryCatch({
          ...
          cat("Error downloading SILVA taxonomy file:", conditionMessage(e), "\n")
          stop("Cannot proceed: Failed to obtain SILVA reference files")
        })
      }
      ...
      cat("Using SILVA references from DADA2 package\n")
    }
    
    # Assign taxonomy using DADA2's method (with confidence scores)
    cat("Assigning taxonomy using DADA2's assignTaxonomy...\n")
    taxa_result <- assignTaxonomy(seqtab.nochim, silva_train, multithread=TRUE, outputBootstraps=TRUE)
    
    # Extract taxonomic assignments and bootstrap confidence values
    taxa_idtaxa <- taxa_result$tax
    taxa_idtaxa_confidence <- taxa_result$boot
    
    # Add species-level assignments
    cat("Adding species-level assignments...\n")
    taxa_idtaxa <- addSpecies(taxa_idtaxa, silva_species, allowMultiple=FALSE, tryRC=TRUE, verbose=TRUE)
    
    # Report success
    cat("Taxonomy assignment completed using DADA2's methods as fallback for IDTAXA\n")
  }
  
  # Only proceed with IDTAXA classification if we didn't use the fallback
  if (!used_fallback) {
    ...
    # IDTAXA Configuration - with lower confidence threshold for debugging
    # Default threshold was 50, lowering to 40 for debugging empty classifications
    idtaxa_confidence_threshold <- 40  # Lower threshold for debugging
    cat(sprintf("Using confidence threshold of %d%% for IDTAXA classification (lowered for debugging)\n", 
                idtaxa_confidence_threshold))
    ...
    # Debug: Verify sequences are valid for classification
    cat("Checking sequences for potential issues...\n")
    seq_lengths <- width(dna_seqs)
    cat("Sequence length summary - Min:", min(seq_lengths), 
        "Max:", max(seq_lengths), 
        "Mean:", round(mean(seq_lengths), 1), "\n")
    
    # Check for any degenerate sequences
    degenerate_bases <- sapply(as.character(dna_seqs), function(s) 
      sum(stringr::str_count(s, "[^ACGT]")))
    if (any(degenerate_bases > 0)) {
      cat("WARNING:", sum(degenerate_bases > 0), "sequences contain degenerate bases.\n")
    }
  
      # Function to test IDTAXA on a small subset first
    test_idtaxa_classification <- function(seqs, training_set, threshold) {
    # Take a small subset for testing
    test_size <- min(5, length(seqs))
    test_seqs <- seqs[1:test_size]
    
    # Run test classification
    cat("Testing IDTAXA classification with", test_size, "sequences...\n")
    test_result <- tryCatch({
      DECIPHER::IdTaxa(test_seqs, training_set, strand = "both", threshold = threshold)
    }, error = function(e) {
      cat("ERROR in test classification:", conditionMessage(e), "\n")
      return(NULL)
    })
    
    # Check results
    if (!is.null(test_result)) {
      # Check for taxonomic assignments
      has_taxonomy <- sapply(test_result, function(x) length(x$taxon) > 0)
      if (all(has_taxonomy)) {
        cat("Test successful: All test sequences received taxonomic classifications.\n")
        # Check confidence values
        conf_values <- lapply(test_result, function(x) x$confidence)
        avg_conf <- mean(unlist(conf_values))
        cat("Average confidence score:", round(avg_conf, 1), "%\n")
        return(TRUE)
      } else {
        cat("WARNING: Some test sequences (", sum(!has_taxonomy), "/", test_size, 
            ") did not receive taxonomic classifications.\n")
        cat("This may indicate issues with the database or sequences.\n")
        return(FALSE)
      }
    } else {
      cat("Test classification failed.\n")
      return(FALSE)
    }
  }
  
  # Run test classification
  test_success <- test_idtaxa_classification(dna_seqs, trainingSet, idtaxa_confidence_threshold)
  
  # Initialize results list
  ids <- NULL
  
  # Proceed with full classification if test was successful, or try with even lower threshold if it failed
  if (!test_success) {
    cat("Initial test failed. Trying with an even lower threshold...\n")
    idtaxa_confidence_threshold <- 30
    test_success <- test_idtaxa_classification(dna_seqs, trainingSet, idtaxa_confidence_threshold)
    
    if (!test_success) {
      cat("WARNING: Test classification still failed with lower threshold.\n")
      cat("Proceeding with full classification anyway, but results may be incomplete.\n")
    } else {
      cat("Test succeeded with threshold =", idtaxa_confidence_threshold, "\n")
    }
  }
  ...
  if(length(dna_seqs) > 3000) {
    ...
    for(i in 1:n_batches) {
      ...
      # Run IDTAXA on this batch with error handling
      batch_ids <- tryCatch({
        DECIPHER::IdTaxa(batch_seqs, trainingSet, strand = "both", 
                        processors = num_processors, threshold = idtaxa_confidence_threshold)
      }, error = function(e) {
        cat("ERROR in batch", i, ":", conditionMessage(e), "\n")
        # Return NULL for each sequence in this batch to avoid breaking the pipeline
        empty_list <- vector("list", length(batch_seqs))
        names(empty_list) <- names(batch_seqs)
        return(empty_list)
      })
      
      # Count successful classifications in this batch
      batch_success <- sum(sapply(batch_ids, function(x) !is.null(x) && length(x$taxon) > 0))
      batch_empty <- length(batch_ids) - batch_success
      
      success_count <- success_count + batch_success
      empty_count <- empty_count + batch_empty
      
      if (batch_empty > 0) {
        cat("WARNING:", batch_empty, "sequences in batch", i, "received no taxonomic classification\n")
      }
    ...
    cat("IDTAXA classification complete:", success_count, "sequences classified successfully\n")
    if (empty_count > 0) {
      cat("WARNING:", empty_count, "sequences (", 
          round(empty_count/length(dna_seqs)*100, 1), 
          "%) received no taxonomic classification\n")
    }
  } else {
    # For smaller datasets, use standard approach with error handling
    ids <- tryCatch({
      DECIPHER::IdTaxa(dna_seqs, trainingSet, strand = "both", 
                      processors = num_processors, threshold = idtaxa_confidence_threshold)
    }, error = function(e) {
      cat("ERROR in IDTAXA classification:", conditionMessage(e), "\n")
      return(NULL)
    })
    ...
    if (!is.null(ids)) {
      ...
      success_count <- sum(sapply(ids, function(x) length(x$taxon) > 0))
      empty_count <- length(ids) - success_count
      
      cat("IDTAXA classification complete:", success_count, "sequences classified successfully\n")
      if (empty_count > 0) {
        cat("WARNING:", empty_count, "sequences (", 
            round(empty_count/length(ids)*100, 1), 
            "%) received no taxonomic classification\n")
      }
    } else {
      cat("ERROR: IDTAXA classification failed completely.\n")
  ...
  # Modified to use lower per-rank confidence thresholds
  convert_idtaxa_to_table <- function(ids) {
    # Get all possible ranks
    all_ranks <- unique(unlist(lapply(ids, function(x) names(x$taxon))))
    if (length(all_ranks) == 0) {
      cat("ERROR: No taxonomic ranks found in IDTAXA results.\n")
      # Return empty data frame with standard columns to avoid breaking pipeline
      empty_df <- data.frame(matrix(NA, nrow = length(ids), ncol = 7))
      colnames(empty_df) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")
      rownames(empty_df) <- names(ids)
      return(empty_df)
    }
    ...
    # Use more relaxed confidence thresholds for lower taxonomic ranks
    confidence_thresholds <- list(
      rootrank = 40,  # Kingdom level
      domain = 40,    # Also kingdom level
      Kingdom = 40,
      Phylum = 35,
      Class = 30,
      Order = 25,
    ...
    empty_assignments <- 0
    assignment_counts <- setNames(rep(0, length(all_ranks)), all_ranks)
    
    for (i in seq_along(ids)) {
      id <- ids[[i]]
      
      # Skip if the ID result is empty
      if (is.null(id) || length(id$taxon) == 0) {
        empty_assignments <- empty_assignments + 1
        next
      }
      
      # Track assignment counts by rank
      for (rank in names(id$taxon)) {
        assignment_counts[rank] <- assignment_counts[rank] + 1
      }
      
      # Process each taxonomic rank
      for (rank in names(id$taxon)) {
        # Get confidence threshold for this rank
        threshold <- if (rank %in% names(confidence_thresholds)) {
          confidence_thresholds[[rank]]
        } else {
          idtaxa_confidence_threshold  # Use default if rank not in thresholds
        }
        
        # Only keep assignments with confidence above threshold for this rank
        if (!is.null(id$confidence) && !is.null(id$confidence[rank]) && 
            !is.na(id$confidence[rank]) && id$confidence[rank] >= threshold) {
          taxa_df[i, rank] <- id$taxon[rank]
        }
      }
    }
    
    # Report assignment statistics by rank
    cat("\nIDTAXA Assignment Statistics by Rank:\n")
    for (rank in names(assignment_counts)) {
      if (assignment_counts[rank] > 0) {
        pct <- round(assignment_counts[rank] / length(ids) * 100, 1)
        threshold <- if (rank %in% names(confidence_thresholds)) {
          confidence_thresholds[[rank]]
        } else {
          idtaxa_confidence_threshold
        }
        cat(sprintf("  %s: %d assignments (%g%%) - threshold: %d%%\n", 
                   rank, assignment_counts[rank], pct, threshold))
      }
    }
    
    if (empty_assignments > 0) {
      cat("  Empty Results:", empty_assignments, "sequences (", 
          round(empty_assignments/length(ids)*100, 1), 
          "%) had no taxonomic assignments\n")
    }
  ...
  # Validate the taxonomy results
  empty_rows <- rowSums(is.na(taxa_idtaxa)) == ncol(taxa_idtaxa)
  if (sum(empty_rows) > 0) {
    cat("WARNING:", sum(empty_rows), "sequences (", 
        round(sum(empty_rows)/nrow(taxa_idtaxa)*100, 1), 
        "%) have no taxonomic classification at any level\n")
    
    # Save problematic sequences for further analysis
    if (sum(empty_rows) > 0) {
      empty_seqs <- dna_seqs[empty_rows]
      if (!dir.exists("results")) dir.create("results")
      saveRDS(empty_seqs, "results/idtaxa_unclassified_sequences.rds")
      cat("Saved unclassified sequences to results/idtaxa_unclassified_sequences.rds for further analysis\n")
    }
  }
  
  # Save taxonomic confidence scores for each assignment
  if (!is.null(ids) && length(ids) > 0) {
    # Extract confidence scores for each ASV and each taxonomic level
    confidence_data <- lapply(ids, function(id) {
      if (is.null(id) || is.null(id$confidence) || length(id$confidence) == 0) {
        return(NULL)
      }
    ...
    if (!dir.exists("results")) dir.create("results")
    saveRDS(confidence_data, "results/idtaxa_confidence_scores.rds")
    cat("Saved detailed confidence scores to results/idtaxa_confidence_scores.rds\n")
  }
  } # End of if (!used_fallback) block
  ...
  # Create the plot with appropriate subtitle depending on method used
  plot_subtitle <- if (used_fallback) {
    "Using DADA2's assignTaxonomy (IDTAXA fallback)"
  } else {
    paste0("Using confidence threshold of ", idtaxa_confidence_threshold, "% (lowered for debugging)")
  }
  ...
  # Save a note about the method used for reference
  if (used_fallback) {
    if (!dir.exists("results")) dir.create("results")
    cat("Used DADA2's assignTaxonomy as fallback due to IDTAXA issues\n", 
        file="results/taxonomy_method_note.txt")
  }
}
```
