...
standardize_taxonomy <- function(taxa_df, method) {
  ...
  cat("Method:", method, "\n")
  cat("Original columns:", paste(colnames(taxa_df), collapse=", "), "\n")
  ...
  cat("Standardized columns:", paste(colnames(result_df), collapse=", "), "\n")
...
summarize_taxonomy <- function(taxa_list) {
  ...
  for (method_name in names(taxa_list)) {
    ...
    cat("Original dimensions:", paste(dim(taxa_df), collapse="×"), "\n")
    cat("Original columns:", paste(colnames(taxa_df), collapse=", "), "\n")
    ...
    for(level in standard_levels) {
      if(level %in% colnames(taxa_df)) {
        ...
        percent <- 0
        cat("Warning: Level", level, "not found in method", method_name, "\n")
      }
    ...
    cat("Processed", method_name, "successfully\n")
  }
...
if (length(taxa_results) > 1) {
  ...
  # Debug: Print information about taxa_results
  cat("Number of taxonomy methods:", length(taxa_results), "\n")
  cat("Method names:", paste(names(taxa_results), collapse=", "), "\n")
  
  # Debug: Print dimensions of each taxonomy result
  for(method in names(taxa_results)) {
    cat("Method:", method, "Dimensions:", paste(dim(taxa_results[[method]]), collapse="×"), "\n")
  }
    ...
    if (exists("parallel_cores") && parallel_cores > 1 && 
      ...
      for (chunk_idx in seq_along(depth_chunks)) {
        ...
        chunk_results <- future.apply::future_lapply(chunk, function(depth) {
          ...
          richness <- suppressWarnings(vegan::rarefy(abundances, depth))
          
          # Create results for each sample with safety checks
          sample_names <- rownames(abundances)
          
          # Handle potential length mismatches for safer data frame creation
          if (length(richness) != length(sample_names)) {
            # Use which() instead of logical indexing to avoid "logical subscript too long" errors
            valid_indices <- which(!is.na(richness))
            
            if (length(valid_indices) > 0) {
              result_df <- data.frame(
                depth = depth,
                richness = richness[valid_indices],
                sample = sample_names[valid_indices],
                stringsAsFactors = FALSE
              )
            } else {
              # No valid samples for this depth
              result_df <- data.frame(
                depth = numeric(0), 
                richness = numeric(0), 
                sample = character(0),
            ...
            # Normal case - create data frame then filter NA values
            result_df <- data.frame(
              depth = depth,
              richness = richness,
              sample = sample_names,
            ...
            # Use which() for safer NA filtering
            result_df <- result_df[which(!is.na(result_df$richness)), ]
          }
          return(result_df)
        })
      ...
      for (i in seq_along(depths)) {
        ...
        # Calculate richness at this depth for each sample with warnings suppressed
        richness <- suppressWarnings(vegan::rarefy(abundances, depth))
        
        # Create results with safety checks
        sample_names <- rownames(abundances)
        
        # Handle potential length mismatches
        if (length(richness) != length(sample_names)) {
          # Get valid indices using which() to avoid "logical subscript too long" errors
          valid_indices <- which(!is.na(richness))
          
          if (length(valid_indices) > 0) {
            # Create a data frame for valid samples directly
            ...
            rarefaction_curves <- rbind(rarefaction_curves, depth_data)
          }
        } else {
          # Normal case - create single data frame for all valid samples at this depth
          valid_indices <- which(!is.na(richness))
          if (length(valid_indices) > 0) {
            depth_data <- data.frame(
              depth = depth,
              richness = richness[valid_indices],
              sample = rownames(abundances)[valid_indices],
              stringsAsFactors = FALSE
            )
