...
efficient_quality_profile <- function(fastq_files, max_files = 3, max_reads = 10000) {
  ...
  if (requireNamespace("future.apply", quietly = TRUE)) {
    # Process each file in parallel with sampling
    cat("Using parallel processing for quality profiles...\n")
    
    # Set up parallel processing plan if not already set
    if (!inherits(future::plan(), "multisession")) {
      future::plan(future::multisession, workers = min(n_files, future::availableCores() - 1))
    }
    
    # Process files in parallel with sampling
    qual_plots <- future.apply::future_lapply(sampled_files, function(file) {
      # Sample reads for faster processing
      dada2::plotQualityProfile(file, n = max_reads, aggregate = TRUE)
    })
    
    # Combine into a single plot with future::value to ensure proper extraction
    combined_plot <- do.call(gridExtra::grid.arrange, c(lapply(qual_plots, future::value), ncol = 1))
    return(combined_plot)
  } else {
    # Fall back to standard sequential processing with sampling
    cat("Using sequential processing with sampling for quality profiles...\n")
