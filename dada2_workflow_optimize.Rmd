  ...
  # Setup for IDTAXA/DECIPHER SILVA classifier
    ...
    if (!file.exists(silva_idtaxa) || force_download) {
      ...
      download_urls <- c(
        ...
        "https://zenodo.org/record/3986799/files/SILVA_SSU_r138_2019.RData",
        "https://github.com/benjjneb/dada2/files/10403520/SILVA_SSU_r138_2019.RData.gz"
      )
      ...
      if (!download_success) {
        ...
        if (requireNamespace("DECIPHER", quietly = TRUE)) {
          ...
          tryCatch({
            ...
            seed_url <- "https://zenodo.org/record/3986799/files/SILVA_SEED_r138.RData"
...
if (taxonomy_methods$IDTAXA_SILVA) {
  ...
  if ("trainingSet" %in% training_objs) {
    ...
    if (!exists("trainingSet") || !inherits(trainingSet, "TrainingSet")) {
      ...
      # Look for potential raw data to build a training set
      if (any(c("sequences", "taxonomy", "aln") %in% training_objs)) {
        cat("Found raw data that might be used to build a training set\n")
        
        # Try to extract sequences and taxonomy
        sequences <- NULL
        taxonomy <- NULL
        
        # Check various possible combinations
        if (all(c("sequences", "taxonomy") %in% training_objs)) {
          sequences <- training_env$sequences
          taxonomy <- training_env$taxonomy
        } else if ("aln" %in% training_objs && "taxonomy" %in% training_objs) {
          sequences <- training_env$aln
          taxonomy <- training_env$taxonomy
        }
        
        # Try to build training set if we have the necessary components
        if (!is.null(sequences) && !is.null(taxonomy)) {
          cat("Attempting to build training set from raw data...\n")
          tryCatch({
            trainingSet <- DECIPHER::LearnTaxa(sequences, taxonomy)
            cat("Successfully built new training set!\n")
            
            # Save for future use
            save(trainingSet, file=idtaxa_files$train)
            cat("Saved rebuilt training set for future use\n")
          }, error = function(e) {
            cat("Failed to build training set:", conditionMessage(e), "\n")
            stop("Cannot proceed: Unable to create valid training set")
          })
        } else {
          stop("Cannot proceed: No valid training set found and couldn't build one")
        }
      } else {
        stop("Cannot proceed: No valid training set or raw data found")
      }
    }
  }
  ...
  if (!exists("trainingSet") || !inherits(trainingSet, "TrainingSet")) {
    stop("Cannot proceed: No valid IDTAXA training set available despite attempts to fix the issue")
  }
  ...
  idtaxa_plot <- ggplot(idtaxa_summary, aes(x = TaxonomicLevel, y = PercentAssigned)) +
    ...
    labs(title = "IDTAXA: Percentage of ASVs with Taxonomic Assignment",
         subtitle = paste0("Using confidence threshold of ", idtaxa_confidence_threshold, "% (lowered for debugging)"),
         x = "Taxonomic Level",
         y = "Percent Assigned") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
