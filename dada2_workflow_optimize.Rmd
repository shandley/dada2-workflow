    ...
    if (exists("parallel_cores") && parallel_cores > 1 && 
      ...
      for (chunk_idx in seq_along(depth_chunks)) {
        ...
        chunk_results <- future.apply::future_lapply(chunk, function(depth) {
          ...
          # Handle cases where the depth is greater than some samples' total counts
          valid_samples <- rowSums(abundances) >= depth
          
          if (sum(valid_samples) == 0) {
            # Skip when no samples have enough reads for this depth
            ...
            # Only calculate for samples with sufficient reads
            valid_abundances <- abundances[valid_samples, , drop = FALSE]
            valid_sample_names <- rownames(valid_abundances)
            
            # Now safely calculate richness only for valid samples
            richness <- suppressWarnings(vegan::rarefy(valid_abundances, depth))
            
            # Create results dataframe directly from valid results
            result_df <- data.frame(
              depth = rep(depth, length(richness)),
              richness = richness,
              sample = valid_sample_names,
              stringsAsFactors = FALSE
            )
            
            # Filter any remaining NA values (should be rare with this approach)
            if (any(is.na(result_df$richness))) {
      ...
      for (i in seq_along(depths)) {
        ...
        if (sum(valid_samples) > 0) {
          ...
          # Create a data frame for valid samples
          ...
          # Filter any NA values (should be rare with this approach)
          if (any(is.na(depth_data$richness))) {
            depth_data <- depth_data[!is.na(depth_data$richness), ]
          }
          
          # Only add to master data frame if we have valid results
