...
# Create tracking table with all processing steps
# First check that we have the same samples throughout
# Get common sample names across all steps
...
# Warn if any samples were dropped
if (length(common_samples) < length(names(derepFs))) {
  cat("Warning: Some samples are missing from one or more processing steps.\n")
  cat("Using only", length(common_samples), "samples for tracking.\n")
}

# Create tracking table explicitly for each sample to avoid dimension issues
track <- data.frame(
  Input = out[common_samples, "reads.in"],
  Filtered = out[common_samples, "reads.out"],
  Denoised_F = sapply(dadaFs[common_samples], getN),
  Denoised_R = sapply(dadaRs[common_samples], getN),
  Merged = sapply(mergers[common_samples], getN)
)

# Add non-chimeric reads column - handle potential dimension mismatch
track$NonChimeric <- 0  # Initialize with zeros
for (sample in rownames(track)) {
  if (sample %in% rownames(seqtab.nochim)) {
    track[sample, "NonChimeric"] <- sum(seqtab.nochim[sample, ])
  }
}

# Ensure row names are preserved
rownames(track) <- common_samples

# Convert to data frame for better display
track_df <- as.data.frame(track)
track_df$Sample <- rownames(track_df)

# Calculate percentage retained at each step
track_df$Percent_Filtered <- round(track_df$Filtered / track_df$Input * 100, 1)
track_df$Percent_Denoised_F <- round(track_df$Denoised_F / track_df$Filtered * 100, 1)
track_df$Percent_Merged <- round(track_df$Merged / track_df$Denoised_F * 100, 1)
track_df$Percent_NonChimeric <- round(track_df$NonChimeric / track_df$Merged * 100, 1)
track_df$Percent_Final <- round(track_df$NonChimeric / track_df$Input * 100, 1)
...
# Create a summary of read loss at each step
pipeline_summary <- data.frame(
  Step = c("Raw Input", "After Filtering", "After Denoising", "After Merging", "After Chimera Removal"),
  Reads = c(sum(track_df$Input), sum(track_df$Filtered), sum(track_df$Denoised_F), 
            sum(track_df$Merged), sum(track_df$NonChimeric)),
  PercentOfInput = c(100, 
                     round(sum(track_df$Filtered) / sum(track_df$Input) * 100, 1),
                     round(sum(track_df$Denoised_F) / sum(track_df$Input) * 100, 1),
                     round(sum(track_df$Merged) / sum(track_df$Input) * 100, 1),
                     round(sum(track_df$NonChimeric) / sum(track_df$Input) * 100, 1)),
  PercentOfPreviousStep = c(100,
                          round(sum(track_df$Filtered) / sum(track_df$Input) * 100, 1),
                          round(sum(track_df$Denoised_F) / sum(track_df$Filtered) * 100, 1),
                          round(sum(track_df$Merged) / sum(track_df$Denoised_F) * 100, 1),
                          round(sum(track_df$NonChimeric) / sum(track_df$Merged) * 100, 1))
)

kable(pipeline_summary, caption = "Summary of Reads Retained at Each Processing Step")

# Create a plot to visualize read tracking
...
avg_tracking <- aggregate(Reads ~ Step, data = track_long, FUN = mean)
avg_tracking$StdDev <- aggregate(Reads ~ Step, data = track_long, FUN = sd)$Reads

avg_tracking_plot <- ggplot(avg_tracking, aes(x = Step, y = Reads, group = 1)) +
  geom_line(size = 1.5, color = "blue") +
  geom_point(size = 3, color = "blue") +
  geom_errorbar(aes(ymin = Reads - StdDev, ymax = Reads + StdDev), width = 0.2, color = "blue") +
  theme_minimal() +
...
# Save the tracking data for dashboard visualization
