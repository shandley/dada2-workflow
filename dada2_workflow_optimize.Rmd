...
# Create checkpoint after filter and trim step
...
# Create checkpoint after learning error rates
...
if (length(derepFs) > 20) {
  ...
  for (i in seq_along(batch_groups)) {
    ...
    dadaFs_batch <- dada(derepFs[batch_samples], err = errF, multithread = TRUE, pool = "pseudo")
...
if (length(derepRs) > 20) {
  ...
  for (i in seq_along(batch_groups)) {
    ...
    batch_samples <- batch_groups[[i]]
    dadaRs_batch <- dada(derepRs[batch_samples], err = errR, multithread = TRUE, pool = "pseudo")
...
# Create checkpoint after sample inference step
...
# Create checkpoint after merging reads step
...
# Create checkpoint after ASV table construction step
...
# Create checkpoint after chimera removal step
...
if (taxonomy_methods$SILVA_DADA2) {
  ...
  # Assign taxonomy at the genus level with optimized parallelization
  cat("Assigning taxonomy using Silva database with optimized parallelization...\n")
  ...
  if(ncol(seqtab.nochim) > 3000) {
    ...
    for(i in 1:n_batches) {
      ...
      # Assign taxonomy for this batch
      batch_taxa <- assignTaxonomy(batch_seqtab, silva_files$train, multithread=TRUE)
      
      # Add species assignments
      batch_taxa <- addSpecies(batch_taxa, silva_files$species)
      
      # Combine with previous results
      if(is.null(taxa_silva)) {
        taxa_silva <- batch_taxa
      } else {
        taxa_silva <- rbind(taxa_silva, batch_taxa)
      }
...
if (can_build_trees) {
  ...
  # Create checkpoint after tree building
  save_checkpoint("after_tree_building", 
                 c("asv_tree", "seqtab.nochim", "taxa"),
                 overwrite = TRUE)
...
if (can_do_rarefaction) {
  ...
  if(!is.null(rarefaction_curves)) {
    ...
    # Create checkpoint after rarefaction analysis step
