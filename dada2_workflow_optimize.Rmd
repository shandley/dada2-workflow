  ...
  tryCatch({
    if (.Platform$OS.type == "unix") {
      if (Sys.info()["sysname"] == "Darwin") {  # macOS
        ...
        # On macOS, also include inactive memory as it can be reclaimed
        inactive_info <- system("vm_stat | grep 'Pages inactive:'", intern = TRUE)
        inactive_pages <- as.numeric(gsub("Pages inactive:\\s+", "", inactive_info))
        inactive_mem_mb <- (inactive_pages * page_size) / (1024^2)
        
        # Total available = free + inactive
        return(free_mem_mb + inactive_mem_mb)
      } else {  # Linux
        mem_info <- system("free -m | grep 'Mem:'", intern = TRUE)
        mem_parts <- strsplit(mem_info, "\\s+")[[1]]
        # On newer Linux, position 7 is "available" which is better than just "free"
        if (length(mem_parts) >= 7) {
          free_mem_mb <- as.numeric(mem_parts[7])
        } else {
          free_mem_mb <- as.numeric(mem_parts[4])
        }
        return(free_mem_mb)
      }
...
# Note: This function has been removed as it's not currently used in the workflow
# If needed in the future, it can be implemented for hardware-accelerated matrix operations
...
if(ncol(seqtab) > 1000) {
  ...
  for(i in 1:n_batches) {
    ...
    if (i < n_batches) {
      current_memory <- get_available_memory()
      memory_threshold <- 0.3 * current_memory  # 30% of available memory as threshold
...
taxonomy_methods <- list(
  ...
  IDTAXA = TRUE            # DECIPHER's IDTAXA implementation
)
...
```{r idtaxa-taxonomy}
# DECIPHER's IDTAXA method for taxonomy assignment
if (taxonomy_methods$IDTAXA) {
  cat("\n=== Method 3: DECIPHER's IDTAXA algorithm ===\n")
  
  # Check if DECIPHER package is available
  if (!requireNamespace("DECIPHER", quietly = TRUE)) {
    cat("DECIPHER package is required for IDTAXA. Please install it with BiocManager::install('DECIPHER')\n")
  } else {
    # Function to check and download IDTAXA training set if needed
    get_idtaxa_training_set <- function() {
      # Check for pre-made IDTAXA training set
      idtaxa_dir <- file.path("ref_db", "idtaxa")
      idtaxa_training_file <- file.path(idtaxa_dir, "SILVA_SSU_r138_2_2024.RData")
      
      if (file.exists(idtaxa_training_file)) {
        cat("Using existing IDTAXA training set:", idtaxa_training_file, "\n")
        # Load training set
        idtaxa_train <- NULL
        load(idtaxa_training_file) # Loads 'trainingSet' object
        return(trainingSet)
      } else {
        # Create idtaxa directory if it doesn't exist
        if (!dir.exists(idtaxa_dir)) {
          dir.create(idtaxa_dir, recursive = TRUE)
        }
        
        # Try to download pre-formatted IDTAXA training set from the DECIPHER website
        idtaxa_url <- "https://www2.decipher.codes/data/Downloads/TrainingSets/SILVA_SSU_r138_2_2024.RData"
        
        tryCatch({
          cat("Downloading pre-formatted IDTAXA training set from DECIPHER website...\n")
          options(timeout = max(300, getOption("timeout")))
          download.file(idtaxa_url, idtaxa_training_file, mode = "wb")
          
          # Check that download was successful and file exists
          if (file.exists(idtaxa_training_file)) {
            cat("IDTAXA training set downloaded successfully\n")
            load(idtaxa_training_file) # Loads 'trainingSet' object
            return(trainingSet)
          } else {
            cat("Failed to download IDTAXA training set\n")
            return(NULL)
          }
        }, error = function(e) {
          cat("Error downloading IDTAXA training set from DECIPHER website:", conditionMessage(e), "\n")
          cat("You can manually download the file from: https://www2.decipher.codes/Downloads.html\n")
          cat("and save it to:", idtaxa_training_file, "\n")
          return(NULL)
        })
      }
    }
    
    # Try first approach: Use pre-formatted training set (most reliable)
    idtaxa_train <- get_idtaxa_training_set()
    
    # If that fails, try alternative IDTAXA training sets
    if (is.null(idtaxa_train)) {
      # Try alternative version (r138_1)
      cat("Trying alternative IDTAXA training set version...\n")
      idtaxa_dir <- file.path("ref_db", "idtaxa")
      idtaxa_training_file <- file.path(idtaxa_dir, "SILVA_SSU_r138_1_2023.RData")
      ...
      if (!file.exists(idtaxa_training_file)) {
        ...
        idtaxa_url <- "https://www2.decipher.codes/data/Downloads/TrainingSets/SILVA_SSU_r138_1_2023.RData"
        
        tryCatch({
          cat("Downloading alternative IDTAXA training set version...\n")
          options(timeout = max(300, getOption("timeout")))
          download.file(idtaxa_url, idtaxa_training_file, mode = "wb")
          
          if (file.exists(idtaxa_training_file)) {
            cat("Alternative IDTAXA training set downloaded successfully\n")
            load(idtaxa_training_file) # Loads 'trainingSet' object
            idtaxa_train <- trainingSet
          }
        }, error = function(e) {
          cat("Error downloading alternative IDTAXA training set:", conditionMessage(e), "\n")
        })
      } else {
        cat("Using existing alternative IDTAXA training set:", idtaxa_training_file, "\n")
        load(idtaxa_training_file) # Loads 'trainingSet' object
        idtaxa_train <- trainingSet
      }
    }
    
    # If that still fails and SILVA database is available, try to create our own
    if (is.null(idtaxa_train) && exists("silva_files") && file.exists(silva_files$train)) {
      cat("Using existing SILVA files to create custom IDTAXA training set...\n")
      
      # Load the SILVA training set into DECIPHER format
      cat("Loading SILVA training data for IDTAXA (this may take a few minutes)...\n")
      silva_train_file <- silva_files$train
    } 
    # If SILVA isn't available but RDP is, use that instead
    else if (is.null(idtaxa_train) && exists("rdp_files") && file.exists(rdp_files$train)) {
      cat("SILVA files not available. Using existing RDP files to create custom IDTAXA training set...\n")
      
      # Load the RDP training set into DECIPHER format
      cat("Loading RDP training data for IDTAXA (this may take a few minutes)...\n")
      silva_train_file <- rdp_files$train
      
      # Use try-catch to handle potential errors gracefully
      tryCatch({
        # Load training set
        trainingSet <- DECIPHER::ReadDNAStringSet(silva_train_file)
        
        # IDTAXA expects sequences in the format ">taxon<tab>sequence"
        # We need to separate the taxonomy information from the sequence IDs
        ids <- names(trainingSet)
        
        # Extract taxonomic information from the sequence headers
        # This assumes SILVA format with semicolon-delimited taxonomy
        taxid <- sapply(strsplit(ids, "\\s"), `[`, 1)
        rank <- gsub("^.*\\;|\\s+.*$", "", ids) # Get last rank
        ...
        # Format the taxonomy strings for IDTAXA
        # Add root as the top level
        taxonomy <- gsub(";$", "", ids)
        taxonomy <- paste0("Root;", taxonomy)
        
        # Ensure tax paths are properly formatted
        taxonomy <- gsub(";", ";", taxonomy) # Standardize delimiters
        
        # Create the IDTAXA-compatible training set
        idtaxa_train <- DECIPHER::LearnTaxa(trainingSet, taxonomy)
        
        cat("IDTAXA training set created successfully\n")
        ...
        idtaxa_training_file <- file.path(idtaxa_dir, "SILVA_custom.RData")
        trainingSet <- idtaxa_train  # Standardize variable name for saving
        
        # Try to save with error handling
        save_result <- tryCatch({
          save(trainingSet, file = idtaxa_training_file)
          TRUE
        }, error = function(e) {
          cat("Error saving IDTAXA training set:", conditionMessage(e), "\n")
          FALSE
        })
        
        if (save_result) {
          cat("Saved IDTAXA training set for future use at:", idtaxa_training_file, "\n")
        } else {
          cat("Warning: Unable to save IDTAXA training set, it will need to be recreated next time\n")
        }
      }, error = function(e) {
        cat("Error creating IDTAXA training set from SILVA:", conditionMessage(e), "\n")
        idtaxa_train <- NULL
      })
    }
    
    # Only proceed if we have a valid training set
    if (!is.null(idtaxa_train)) {
      # Display information about which training set was used
      cat("\nIDTAXA training set successfully loaded.\n")
      
      # Try to extract version info from the training set if available
      taxonomy_source <- tryCatch({
        if (exists("trainingSet") && !is.null(attr(trainingSet, "taxonomy"))) {
          # Extract database version from the taxonomy attribute
          db_info <- attr(trainingSet, "taxonomy")[1]
          if (!is.null(db_info) && length(db_info) > 0) {
            # Extract source info
            if (grepl("SILVA", db_info)) {
              "SILVA database"
            } else if (grepl("RDP", db_info)) {
              "RDP database"
            } else {
              "custom database"
            }
      ...
      cat("Using IDTAXA with", taxonomy_source, "for taxonomy classification.\n")
      
      # Use IDTAXA for classification with hardware optimization
      cat("Classifying ASVs with IDTAXA...\n")
      
      # Create DNAStringSet from ASV sequences
      asv_seqs <- Biostrings::DNAStringSet(colnames(seqtab.nochim))
      names(asv_seqs) <- paste0("ASV", seq_along(asv_seqs))
      
      # Apply hardware optimizations for IDTAXA
      if (exists("hw_caps") && hw_caps$apple_silicon) {
        cat("Applying Apple Silicon optimizations for IDTAXA...\n")
        # For Apple Silicon, use more threads but less RAM per thread
        idtaxa_threads <- max(1, hw_caps$optimal_workers)
        # IDTAXA threshold: 
        # - Setting to 0 allows all taxonomic assignments regardless of confidence
        # - Higher values (e.g., 50-60) require higher confidence, which might result in fewer assignments
        # - We're lowering it to get assignments at all taxonomic levels, even with low confidence
        idtaxa_threshold <- 0 # Allow all taxonomic assignments, regardless of confidence
        cat("IDTAXA using lowest possible threshold to maximize taxonomic assignments\n")
      } else if (exists("hw_caps") && hw_caps$has_gpu) {
        cat("Applying GPU-aware optimizations for IDTAXA...\n")
        # For GPU systems, CPU can use more threads
        idtaxa_threads <- max(1, hw_caps$optimal_workers)
        idtaxa_threshold <- 0 # Allow all taxonomic assignments, regardless of confidence
      } else {
        # Default settings
        idtaxa_threads <- parallel::detectCores() - 1
        idtaxa_threads <- max(1, idtaxa_threads)
        idtaxa_threshold <- 0 # Allow all taxonomic assignments, regardless of confidence
      }
      
      # Run IDTAXA classification with error handling
      tryCatch({
        # Run IDTAXA classification
        taxa_idtaxa_raw <- DECIPHER::IdTaxa(
          asv_seqs, 
          idtaxa_train, 
          strand = "both", 
          threshold = idtaxa_threshold, 
          processors = idtaxa_threads
        )
        
        cat("IDTAXA classification complete\n")
        
        # Show an example of the raw classification format to help with debugging
        if (length(taxa_idtaxa_raw) > 0) {
          cat("Example of raw IDTAXA output for first ASV:\n")
          print(taxa_idtaxa_raw[[1]])
        }
        
        # Convert IDTAXA output to standard taxonomy matrix format
        # IDTAXA returns a list with taxon assignments and confidences
        n_asv <- length(taxa_idtaxa_raw)
        taxa_idtaxa <- matrix(NA, nrow = n_asv, ncol = 7)
        colnames(taxa_idtaxa) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")
        rownames(taxa_idtaxa) <- names(asv_seqs)
        
        # Extract confidence values for reporting
        confidence_idtaxa <- matrix(NA, nrow = n_asv, ncol = 7)
        colnames(confidence_idtaxa) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")
        rownames(confidence_idtaxa) <- names(asv_seqs)
        
        cat("Processing", n_asv, "ASV classifications...\n")
        
        # Process each ASV classification result
        for (i in 1:n_asv) {
          # Get the taxonomic assignments and confidence values for this ASV
          taxon <- taxa_idtaxa_raw[[i]]$taxon
          confidence <- taxa_idtaxa_raw[[i]]$confidence
          
          # Handle "Root;" prefix - don't remove it, as it's the Kingdom level
          # Skip if no classification (should not happen with defaults)
          if (length(taxon) == 0) next
          
          # Replace "Root" with "Bacteria" if we know it's a 16S dataset
          if (grepl("^Root", taxon) && !is.null(primer_info) && 
              grepl("16S|V[1-9]", primer_info, ignore.case=TRUE)) {
            taxon <- sub("^Root", "Bacteria", taxon)
            cat("Replaced Root with Bacteria for taxonomic assignment\n")
          }
          
          # Split the full taxonomy into ranks
          tax_ranks <- strsplit(taxon, ";")[[1]]
          conf_values <- confidence
          
          # Map to standard taxonomy levels - making sure to handle the Root prefix
          ranks <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")
          
          # The first element might be "Root" or we might have replaced it with "Bacteria"
          # Either way, it should map to Kingdom level
          
          # Assign each taxonomic rank and its confidence
          # Start with the first element of tax_ranks as Kingdom, regardless of content
          for (j in 1:min(length(tax_ranks), length(ranks))) {
            taxa_idtaxa[i, ranks[j]] <- tax_ranks[j]
            
            # Store confidence value if available
            if (j <= length(conf_values)) {
              confidence_idtaxa[i, ranks[j]] <- conf_values[j]
            }
          }
          
          # Add a debug message to show first few assignments
          if (i <= 6) {
            cat("ASV", i, "taxonomic assignment:", paste(taxa_idtaxa[i,], collapse=", "), "\n")
          }
        }
        
        # Convert to data frame for easier handling
        taxa_idtaxa <- as.data.frame(taxa_idtaxa)
        confidence_idtaxa <- as.data.frame(confidence_idtaxa)
        ...
        print(head(confidence_idtaxa))
        
        # Save to results list
        taxa_results$IDTAXA <- taxa_idtaxa
        taxa_results$IDTAXA_confidence <- confidence_idtaxa
        ...
        idtaxa_summary <- data.frame(
          ...
          MeanConfidence = c(
            mean(confidence_idtaxa[, "Kingdom"], na.rm = TRUE),
            mean(confidence_idtaxa[, "Phylum"], na.rm = TRUE),
            mean(confidence_idtaxa[, "Class"], na.rm = TRUE),
            mean(confidence_idtaxa[, "Order"], na.rm = TRUE),
            mean(confidence_idtaxa[, "Family"], na.rm = TRUE),
            mean(confidence_idtaxa[, "Genus"], na.rm = TRUE),
            mean(confidence_idtaxa[, "Species"], na.rm = TRUE)
          )
        )
        
        # Display IDTAXA summary table
        kable(idtaxa_summary, caption = "IDTAXA Taxonomic Assignment Summary with Confidence Scores")
        
        # Create a bar plot for IDTAXA results with confidence info
        ...
        idtaxa_plot <- ggplot(idtaxa_summary, aes(x = TaxonomicLevel, y = PercentAssigned)) +
          ...
          labs(title = "IDTAXA: Percentage of ASVs with Taxonomic Assignment",
               ...
               y = "Percent Assigned") +
          theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
          scale_y_continuous(limits = c(0, max(idtaxa_summary$PercentAssigned) * 1.2))
        
        print(idtaxa_plot)
        
      }, error = function(e) {
        cat("Error during IDTAXA taxonomy assignment:", conditionMessage(e), "\n")
        cat("IDTAXA classification not available. Please check DECIPHER installation and database files.\n")
      })
    } else {
      cat("\n======================================================================\n")
      cat("WARNING: No IDTAXA training set could be created or loaded.\n")
      cat("IDTAXA classification is not available for this run.\n\n")
      cat("To fix this issue:\n")
      cat("1. Download IDTAXA training data from: https://www2.decipher.codes/Downloads.html\n")
      cat("2. Save the file (SILVA_SSU_r138_2_2024.RData) to:\n")
      cat("   ", file.path(getwd(), "ref_db", "idtaxa", "SILVA_SSU_r138_2_2024.RData"), "\n")
      cat("3. Re-run this workflow to use IDTAXA for taxonomy classification\n")
      cat("======================================================================\n")
    }
  }
}
...
build_phylogenetic_tree <- function(seqs, method = "ML", bootstrap = 100, cores = NULL, 
  ...
  if (Sys.info()["sysname"] == "Darwin" && exists("hw_caps") && hw_caps$apple_silicon) {
  ...
  } else if (exists("hw_caps") && hw_caps$apple_silicon) {
...
if (can_build_trees) {
  ...
  if (requireNamespace("gpuR", quietly = TRUE) || 
      ...
      (requireNamespace("torch", quietly = TRUE) && 
       exists("hw_caps") && hw_caps$apple_silicon)) {
